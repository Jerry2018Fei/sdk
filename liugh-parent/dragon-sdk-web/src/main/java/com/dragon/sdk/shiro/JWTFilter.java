...
import com.dragon.sdk.util.JWTUtil;
...
/**
 * @author grm
 *     <p>代码的执行流程preHandle->isAccessAllowed->isLoginAttempt->executeLogin
 */
@Slf4j
public class JWTFilter extends BasicHttpAuthenticationFilter {
  ...
  protected boolean isAccessAllowed(
    ...
    if (isLoginAttempt(request, response)) {
      try {
        ...
        e.printStackTrace();
        try {
          responseError(request, (HttpServletResponse) response);
  ...
  private void setUserBean(ServletRequest request, ServletResponse response, JWTToken token) {
    ...
    request.setAttribute("currentUser", adminBean);
  }
  ...
  protected boolean preHandle(ServletRequest request, ServletResponse response) throws Exception {
    ...
    log.info("http请求：{} {} {}", type, uri, query);
  ...
  /** 非法url返回身份错误信息 */
  private void responseError(ServletRequest request, HttpServletResponse response)
      throws IOException {
    ...
    try {
      ...
      out.write(
          JSONObject.toJSONString(
                  ResponseHelper.validationFailure(PublicResultConstant.UNAUTHORIZED))
              .getBytes());
      out.flush();
